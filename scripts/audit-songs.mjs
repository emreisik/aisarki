 ...
 * - Image durumu
 *
 * Not: wrapJob() ile sarılı — her çalıştırmada admin'in /jobs sayfasında
 * son run durumu görünür (DATABASE_URL set olduğu sürece, aksi halde tracking skip).
 */
...
await wrapJob({ name: "audit-songs", mode: "execute" }, async () => {
  ...
  if (Number(s.suno_only) > 0) {
    ...
    for (const r of rows) {
      console.log(
        `  ${r.id} | ${r.title?.slice(0, 40)} | ${r.audio_url.slice(0, 60)}...`,
      );
    }
  }
  ...
  if (Number(s.broken) > 0) {
    ...
    for (const r of rows) {
      console.log(
        `  ${r.id} | ${r.title?.slice(0, 40)} | task=${r.task_id?.slice(0, 12) || "(null)"} | ${r.status}`,
      );
    }
  }

  // wrapJob summary — admin UI'da JSON olarak görünür
  return {
    total: Number(s.total),
    complete: Number(s.complete),
    hasAudioKey: Number(s.has_audio_key),
    sunoOnly: Number(s.suno_only),
    broken: Number(s.broken),
    hasImageKey: Number(s.has_image_key),
    imageSunoOnly: Number(s.image_suno_only),
  };
});
