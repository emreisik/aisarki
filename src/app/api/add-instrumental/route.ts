...
/**
 * Add Instrumental — vokal şarkıya AI ile enstrümantal eşlik ekler
 * POST /api/add-instrumental
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(
      `${SUNO_BASE_URL}/api/v1/generate/add-instrumental`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok || data.code !== 200) {
      const translated = translateSunoError(
        data.code,
        data.message || data.msg || "Enstrümantal ekleme başarısız",
      );
      if (taskId) {
        saveProcessingTask(
          taskId,
