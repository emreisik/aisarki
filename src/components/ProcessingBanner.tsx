...
export default function ProcessingBanner({
  ...
  /** Processing task'ı iptal et (örn 5 dk takılı kalmış) */
      ...
      {activeTasks.length > 0 && (
        <div className="rounded-2xl overflow-hidden border border-[#1db954]/30 bg-gradient-to-br from-[#0d1f14] via-[#0a1810] to-[#0a0a0a] shadow-[0_0_40px_rgba(29,185,84,0.12)]">
          ...
          <div className="px-4 py-4">
            ...
            <div className="flex flex-col gap-3">
              {activeTasks.map((t) => {
                const ageSec = t.startedAt
                  ? Math.floor(
                      (Date.now() - new Date(t.startedAt).getTime()) / 1000,
                    )
                  : 0;
                // 3 dk sonra "takılı kaldı" uyarısı, iptal butonu daha görünür
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                          ...
                          {[0, 0.2, 0.1, 0.3, 0.05].map((d, i) => (
                            <span
                              ...
                              style={{
                                width: "2px",
                                height: "100%",
                                background: isStale ? "#f59e0b" : "#1db954",
                                animationDelay: `${d}s`,
                              }}
                            />
                          ))}
                        ...
                        <span
                          className={`text-xs ${
                            isStale ? "text-[#f59e0b]" : "text-[#1db954]/80"
                          }`}
                        >
                          {isStale
                            ? `Beklenen süreyi aştı (${Math.floor(ageSec / 60)} dk)`
                            : t.attempts != null
                              ? `${t.attempts}. deneme`
                              : "Üretiliyor..."}
                        </span>
                    ...
                    {/* İptal butonu — 30sn sonra görünür (kullanıcı takıldığında çıkış yolu) */}
                    {onCancel && ageSec > 30 && (
                      <button
                        onClick={() => onCancel(t.taskId)}
                        className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-[#a7a7a7] hover:text-white hover:bg-white/10 transition-colors pressable"
                        title="İptal et"
                      >
                        <X size={16} />
                      </button>
